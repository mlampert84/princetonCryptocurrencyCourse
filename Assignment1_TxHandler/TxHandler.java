...
//		          		   System.out.println("New input value: " + currentPool.getTxOutput(currentUTXO).value);
		          		   ...
		          		   //Add the new output to the UTXO
		          		   currentPool.addUTXO(new UTXO(tx.getHash(),i.outputIndex), o);
		          	   }
		          	  
         		      //Output is greater than or equal to input
         		      else{
		          	      o.value -= sourceOutput.value;
		          	      remainingOutput = o.value;
//		          	      System.out.println("Removing input with value: " + sourceOutput.value + ". Output has value " + o.value);
		           	      
		          	      sourceOutput.address = o.address;
		          	      //Add new available output to UTXO
		          	      currentPool.addUTXO(new UTXO(tx.getHash(), currentUTXO.getIndex()), sourceOutput);
		          	      
		          	      //Input is exhausted so remove it from the UTXO pool and the transaction
		          	      currentPool.removeUTXO(currentUTXO);
		          	      txInputs.remove(0);
		          	   	}
         	   }
